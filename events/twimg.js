...
const twiKey = Buffer.from('3nVuSoBZnx6U4vzUxf5w:Bcs59EFbbsdF6Sl9Ng71smgStWEGwXXKSjYvPVt7qys').toString('base64');
...
async function getTweetImages(ctx, snowflake, msg) {
	return new Promise(async (resolve, reject) => {
		...
		let tweet = await ctx.libs.superagent
			.get(`https://api.twitter.com/1.1/statuses/show.json?id=${snowflake}&trim_user=1&include_entities=1`)
		...
		if (tweet.is_quote_status && tweet.quoted_status_id) {
			if (quoted >= 5) return;
			msg.channel
				.createMessage(`Quoted Tweet: https://twitter.com/statuses/${tweet.quoted_status_id_str}`)
				.then(async (x) => {
					x.addReaction('\u274c');
					twimg_embeds[x.id] = msg.id;

					let imgs = await getTweetImages(ctx, tweet.quoted_status_id_str, msg);
					if (imgs.length > 0) {
		...
		if (tweet.extended_entities) {
			if (
				...
				msg.channel
					.createMessage({
						embed: {
							description: `[Twitter Video/GIF File](${vid.video_info.variants.length > 1
...
async function getMastoImages(ctx, url, msg) {
	return new Promise(async (resolve, reject) => {
		...
		let post = await ctx.libs.superagent.get(url).set('Accept', 'application/activity+json').then((x) => x.body);
		if (post.attachment.length > 0 && !post.sensitive) {
			...
			if (vids.length > 0) {
				msg.channel
					.createMessage({
						content: `**Post Content:** ${post.sensitive
							? `Content Warning: ${entities.decode(
									post.summary.replace(/<br>/g, '\n').replace(/<(?:.|\n)*?>/gm, '')
								)}`
							: entities.decode(post.content.replace(/<br>/g, '\n').replace(/<(?:.|\n)*?>/gm, ''))}`,
						embed: {
...
let plembed = async function(msg, ctx) {
	...
	if (msg.channel.permissionsOf(ctx.bot.user.id).has('manageMessages')) {
		ctx.bot.requestHandler
			.request('POST', `/channels/${msg.channel.id}/messages/${msg.id}/suppress-embeds`, true, { suppress: true })
	...
	msg.channel.createMessage({
		embed: {
			...
			url: url,
			description: `${post.attachment && post.attachment.length > 0
			...
			image: {
				url: post.sensitive ? '' : post.attachment && post.attachment.length > 0 ? post.attachment[0].url : ''
			},
			color: 0x282c37
		}
	});
	if (post.attachment && post.attachment.length > 1) {
		msg.channel.createMessage(post.attachment.splice(1).map((x) => x.url).join('\n'));
	}
};
...
async function twimgDelete(msg, emote, uid, ctx) {
	if (twimg_embeds[msg.id] !== undefined) {
		...
		if (
			(m.author.id == uid || (msg.channel.permissionsOf(uid).has('manageMessages') && uid != ctx.bot.user.id)) &&
			emote.name == '\u274c'
		) {
			msg.delete();
